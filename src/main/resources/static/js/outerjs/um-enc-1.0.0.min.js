...
!function() {
    window.getRSAPublicKey = function(data) {
        $.ajax({
            url : openURL + "/um/encrypt.do",
            dataType : 'jsonp',
            jsonp : 'jsonpCallback',
            timeout : 5000, //超时时间5秒
    ...
    if (typeof window.postMessage == 'undefined') {
        var vbs = [
            "Function vbs_getRSAPublicKey(data)",
            "getRSAPublicKey data",
            "End Function",
            'navigator.getRSAPublicKey = GetRef("getRSAPublicKey")'
        ].join("\r\n");
        navigator.getRSAPublicKey = null;
        window.execScript && window.execScript(vbs, "VBScript");
    } else {
        function listener(event) {
            getRSAPublicKey((event || window.event).data);
        }
...
$.RSA = function() {
    ...
    //The maximum number of digits in base 10 you can convert to an integer without JavaScript throwing up on you.
    var dpl10 = 15;
    ...
    var lr10 = biFromNumber(1000000000000000);//lr10 = 10 ^ dpl10
    ...
    function RSAKeyPair(encryptionExponent, decryptionExponent, modulus, keylen){
        setMaxDigits(130);
        // Convert from hexadecimal and save the encryption/decryption exponents and modulus as big integers in the key object.
    ...
    function rsaDecrypt(value){
        var key = new RSAKeyPair("", exponent, modulus);
        return decrypt(key, value).reverse();
    };

    function encrypt(key, s, pad, encoding){
         ...
         *
         * NoPadding - key length PKCS1Padding - key length - 11
         */
        if (padtype == 1) {
            if (sl > key.chunkSize) {
                sl = key.chunkSize;
        ...
        /*
         * Convert the plaintext string to an array of characters so that we can
         ...
         * and pad the remainder of the block. Otherwise, we pad at the front of the
        ...
        for (i = 0; i < al; i += key.chunkSize) {
            ...
            if (encodingtype == 1) {
                ...
                text = (key.radix == 16) ? biToHex(crypt) : biToString(crypt,
                    key.radix);
            }
            result += text;
        }
    ...
    function decrypt(key, c) {
        ...
        for (i = 0; i < blocks.length; ++i) {
            // Depending on the radix being used for the key, convert this cyphertext block into a big integer.
            if (key.radix == 16) {
             ...
             * we are using big integers, each element thereof represents two bytes
             * of plaintext.
             */
...
$.BASE64 = function(){
    ...
    function encode(input){
        ...
        while (i < input.length) {
            ...
            output += _keys.charAt(enc1) + _keys.charAt(enc2) + _keys.charAt(enc3) +_keys.charAt(enc4);
        }
        return output;
    };
...
$.Encrypt = function() {
    var separator = "\n";
    // 先Base64编码,RSA加密
    function RSAEncrypt(u, p, salt) {
        // Request ID + 时间戳 + 盐 + uid + 密码
        var val = /*$.UUID.guid() + separator +*/ new Date().getTime() + separator
            + (salt || "") + separator + u + separator + p;
        return $.RSA.rsaEncrypt($.BASE64.encode(val));
    };
    return {
        RSAEncrypt : RSAEncrypt
    };
...
$.Decrypt = function() {
    ...
    function RSADecrypt(val) {
        return $.BASE64.decode($.RSA.rsaDecrypt(val));
    };
    return {
        RSADecrypt : RSADecrypt
    };
